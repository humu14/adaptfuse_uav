...
C2A~\cite{nihal2024c2a}                  & Crowd / victim            & RGB            & 10{,}215 & Aerial human-detection dataset for disaster scenarios; victim labels from annotations \\
AIDER~\cite{kyrkou2019aider}             & Multi-disaster (fire, flood, collapse, normal) & RGB & 6{,}433 & Benchmark for aerial disaster scene classification \\
SARD~\cite{sambolek2021sard}             & Victim (person visible)   & RGB            & 5{,}755  & Aerial person search dataset; victim\_label=1 per sample \\
ESC-50~\cite{piczak2015esc}             & Audio events              & Audio          & 2{,}000  & Environmental clips reused across 39{,}003 assigned metadata rows; label-matched, not synchronized \\
...
Training large suites of multimodal models on the full-balanced split ($\sim$98{,}556 samples) at batch size 16 on a single RTX 5090 (32\,GB VRAM) would require approximately 3 hours per run and $\sim$80 hours total. A \textit{Stratified Balanced Training Subset} is therefore derived by applying source-stratified 50\% sampling to each disaster class of the full-balanced split, with a minimum of 5 rows per dataset source to ensure no source is dropped. The result is exactly 12{,}319 rows per disaster class, halving training time to $\sim$1.5 hours per run while preserving exact class balance and full source coverage.
...
\begin{itemize}
    \item \textbf{RGB:} MobileNetV3-Small~\cite{howard2019mobilenetv3} pretrained on ImageNet. The 576-dim output of the final squeeze-excitation block is projected to 192 dimensions via a linear layer. Compute: $\sim$57 MFLOPs per frame.
    \item \textbf{Pseudo-thermal:} ResNet-18~\cite{he2016deep} adapted for single-channel input (pretrained weights averaged across RGB channels at the first layer). The 512-dim global average-pooled output is projected to 192 dimensions. All populated thermal paths in our metadata are RGB-derived maps; no sensor-IR inputs are evaluated.
...
\begin{itemize}
    ...
    \item \textbf{PANNS-CNN6 Audio Backbone:} A 4-block convolutional network pretrained on 2.08 million AudioSet clips, producing 512-dimensional acoustic representations.
...
PANNS-CNN6 encoder~\cite{kong2020panns} & 1.55 & 0.213 & -- & 102 & \textbf{Selected} \\
\midrule
...
\begin{enumerate}
    ...
    To resolve multi-task negative transfer, where dense bounding box gradients corrupt the shared classification backbone:
    \begin{itemize}
...
For dense wildfire detection on the D-Fire benchmark, the \textbf{Hazard Prior Gate (HPG)} injects domain-specific physical priors into the detection backbone at stride~4:
\begin{equation}
    \mathbf{P}_{\text{fire}} = \text{ReLU}(2R - G - B), \quad \mathbf{P}_{\text{smoke}} = 1 - \frac{\max(R,G,B) - \min(R,G,B)}{\max(R,G,B) + \epsilon}
\end{equation}
These deterministic chromaticity maps are concatenated with high-frequency spatial gradients and processed via a depthwise gating convolution. In our runs this shortens wall-clock training from 4.45 to 1.96 hours at a cost of 0.94 points of $\mathrm{mAP}_{50}$ (Section~\ref{sec:object_detection}); whether this reflects faster convergence has not been tested with learning curves.
...
Learning rate & $10^{-3}$ with 5-epoch linear warmup $\to$ cosine decay to $10^{-5}$ \\
...
Loss (Distillation) & + logit KD (temp.=4.0) + feature KD ($\lambda=0.5$ each) \\
...
A total of 26 multimodal classification model configurations (plus 7 object detection configurations, totaling 33 models) were trained and evaluated on the RTX 5090 (32\,GB VRAM) platform. All twenty-six classification models were trained on the Stratified Balanced Training Subset (49{,}276 samples), with training times ranging from 1.5 to 3.5 hours per run; the separate grouped-split retraining was not.
